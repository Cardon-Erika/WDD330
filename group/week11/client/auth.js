...
export default class Auth {
  ...
  async login(callback) {
    ...
    try {
      ...
      // since we have a token let's go grab some data from the API by
      // executing the callback if one was passed in
      if (callback) {
        callback();
      }
    } catch (error) {
  ...
  // uses the email of the currently logged in user to pull up the full user
  // details for that user from the database
  async getCurrentUser(email) {
    try {
      // 3. add the code here to make a request for the user identified by
      // email...don't forget to send the token!
